...
export function StatisticsPanel() {
  ...
  const [remoteParticipations, setRemoteParticipations] = useState<any[] | null>(
    null,
  );
  const [systemStats, setSystemStats] = useState<any | null>(null);
  const [userRatingsByEvent, setUserRatingsByEvent] = useState<
    Record<string, number>
  >({});
  const [participantStatsByEvent, setParticipantStatsByEvent] = useState<
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      try {
        ...
        const myEventIds = normalizedEvents
          .filter((e: any) => e.createdBy === currentUser.id)
          .map((e: any) => e.id);
              ...
              myEventIds.map(async (eventId: string) => {
                try {
                  ...
                  const checked = rows.filter(
                    (r: any) => !!(r.checked_in ?? r.checkedIn),
                  ).length;
        ...
        try {
          ...
          const entries = await Promise.all(
            uniqueIds.map(async (eventId: string) => {
              try {
                const res = await axios.get(`${BASE}/chats/reviews/${eventId}`, {
                  headers,
                });
                const list = (res.data?.reviews ?? res.data ?? []) as any[];
                const mine = list.find(
                  (r: any) => (r.user_id ?? r.userId) === currentUser.id,
                );
  ...
  const myParticipations = useMemo(() => {
    if (dataMyParticipations && Array.isArray(dataMyParticipations)) {
      ...
      const eventIdSet = new Set<string>(
        dataMyParticipations.map((r: any) => r.event_id),
      );
      return dataEvents.filter((e: any) => eventIdSet.has(e.id));
    }
    return dataEvents.filter((e: any) =>
      (e.participants ?? []).some((p: any) => p.userId === currentUser.id),
    );
    ...
    return myParticipations.reduce(
      (sum, e) =>
        ...
        e.participants.filter(
          (p: Participant) => p.userId === currentUser.id && p.checkedIn,
        ).length,
      0,
    );
  ...
  const adminStats = [
    ...
    },
    {
      ...
      value:
        systemStats?.approved_events ??
        dataEvents.filter((e: any) => e.status === "approved").length,
      icon: CheckCircle,
      color: "bg-green-500",
    },
    {
      ...
      value:
        systemStats?.pending_events ??
        dataEvents.filter((e: any) => e.status === "pending").length,
      icon: Clock,
      color: "bg-orange-500",
    },
    {
      ...
      value:
        systemStats?.rejected_events ??
        dataEvents.filter((e: any) => e.status === "rejected").length,
      icon: Calendar,
      color: "bg-red-500",
    },
    {
      ...
      value:
        systemStats?.cancelled_events ??
        dataEvents.filter((e: any) => e.status === "cancelled").length,
      icon: XCircle,
      color: "bg-rose-500",
    },
    {
      ...
      value:
        systemStats?.total_participations != null
  ...
  const organizerStats = [
    ...
    },
    {
      ...
      value: myEvents.reduce((sum, e: any) => {
        ...
        const count = Array.isArray(e.participants)
          ? e.participants.length
          : Number(e.current_participants ?? 0);
        return sum + (Number.isFinite(count) ? count : 0);
      }, 0),
      icon: Users,
      color: "bg-purple-500",
  ...
  const userRoleData = [
    {
      name: "Admin",
      value: dataUsers.filter((u: any) => u.role === "admin").length,
    },
    {
      name: "Moderator",
  ...
  const participantsPerEvent = myEvents.map((e) => {
    ...
    const checked = stats
      ? stats.checked
      : Array.isArray(e.participants)
        ? e.participants.filter((p: Participant) => p.checkedIn).length
        : 0;
  ...
  const checkInData = [
    ...
    {
      name: "Chưa điểm danh",
      value: Math.max(0, myParticipations.length - myCheckedIn),
    },
  ];
        ...
        <>
          ...
          <div className="grid grid-cols-[repeat(auto-fit,minmax(350px,1fr))] gap-6">
            {adminStats.map((stat, i) => (
              <div
                key={i}
                className="card p-6 rounded-2xl shadow-lg"
              >
          ...
          <table className="min-w-full text-sm border-separate border-spacing-0">
            ...
            <tbody>
              {myParticipations.map((e) => {
                ...
                const ratingObj = e.ratings.find(
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 border-b border-gray-200 dark:border-dark-border">
                      ...
                      e.participants.find(
                          (p: Participant) =>
                            p.userId === currentUser.id && p.checkedIn,
                        ) ? (
                        <span className="inline-flex items-center gap-1 px-3 py-1 text-sm font-semibold bg-green-100 dark:bg-green-700 text-green-800 dark:text-green-100 rounded-full shadow-sm transition hover:scale-105">
                          <CheckCircle className="w-4 h-4" />
                          Đã Check-in
                        ...
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800">
                          <div className="flex items-center gap-[2px] text-yellow-500">
                            {[1, 2, 3, 4, 5].map((i) => (
                              <Star
                                key={i}
                                className={`w-4 h-4 ${i <= Number(ratingValue) ? "fill-current" : ""}`}
                              />
                            ))}
                          </div>
